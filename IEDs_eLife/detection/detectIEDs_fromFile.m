...
if fDeets.bytes>0
    ...
    if fDeets.bytes<2.5e10 % splitting data into smaller segments if it's too big to read.
        ...
        if exist('IEDdata','var')
            save(['/storage/selected_data/elliotWorking/Data/IEDs/' ptID '/IEDsfromfile_' fName '.mat'],'IEDdata','-v7.3')
        end
        ...
        if iscell(NS5.Data)
            if size(NS5.Data{2},1)<50
                ...
                for chk = 1:nSegs
                    ...
                    if exist('IEDdata','var')
                        save(['/storage/selected_data/elliotWorking/Data/IEDs/' ptID '/IEDsfromfile' fName '__chunk' num2str(chk) '.mat'],'IEDdata','-v7.3')
                    end
                end
            end
