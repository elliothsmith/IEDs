...
%     close(gcf)
    ...
    if exist('IEDdata','var')
        save(['/mnt/mfs/selected_data/elliotWorking/Data/IEDs/' ptID '/IEDsfromfile_' fName '.mat'],'IEDdata','-v7.3')
    end
    ...
    if iscell(NS5.Data)
        if size(NS5.Data{2},1)<50
            ...
            for chk = 1:nSegs
                ...
                [IEDdata] = detectIEDs(double(NS5.Data{2}(1:nChans,:)),3e4);
...
%                 close(gcf)
                ...
                if exist('IEDdata','var')
                    save(['/mnt/mfs/selected_data/elliotWorking/Data/IEDs/' ptID '/IEDsfromfile' fName '__chunk' num2str(chk) '.mat'],'IEDdata','-v7.3')
                end
            end
        end
...
% catch
% fprintf('\ndetection failed for file %s...',fileName)
% end
