...
for ch = 1:nChans
    ...
    % [20180927] :: two clustering approaches didn't really work.
    % ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    %     % clustering data and localizing clusters in time to remove duplicate detections.
    %     % TODO:: deal with channels without IEDs.
    %     % [clusters,isnoise] = DBSCAN(IEDdata.foundPeaks(ch).locs,0.5,1);
    %     clusters = clusterdata(IEDdata.foundPeaks(ch).locs,1);
    %     for cl = 1:max(unique(clusters))
    %         chosenLocs(cl) = median(IEDdata.foundPeaks(ch).locs(clusters==cl));
    %     end
    % ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    
    
    % Doing something simpler, which is just to retain those detections
    % that occur within 250 ms, or 1/4 of the window we are examining.
    % TODO:: this just retains the first detection, though it would be nice
    % to retain the largest detection within a window. Another way to do
    % this would be to realign the data to the max point.
    ...
    if gt(nPeaks(ch),max(nPeaks(1:ch-1)))
        for pk = 1:nPeaks(ch)
            ...
            if and(gt(chosenLocs(pk)-floor(IEDdata.parameters.windowSize*Fs/2),0),lt(chosenLocs(pk)+ceil(IEDdata.parameters.windowSize*Fs/2),length(tmpSig)))
                ...
                IEDdata.fullBWdata(pk).peakData = ...
