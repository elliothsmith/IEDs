    ...
    for fl = 3:length(dirList)
        ...
        for  wp = 1:nWavePoints
            ...
            if isempty(detectionIdcs)
                ...
                if gt(nChans,numel(goodChannels))
                    nChans = numel(goodChannels);
                    wavePointData = wavePointData(goodChannels,:);
                end
                ...
                if plotIEDwaves
                    ...
                    for ch = 1:nChans
                        ...
                        [W,~,scale] = basewave4(wavePointData(ch,:),Fs,fPass(1),fPass(2),6,0);
                        Sft(:,:,ch) = abs((W))./repmat(1./logspace(0,log10(fPass(2)),length(scale))',1,size(W,2));
                    end

                    % determining spectral scales in Hz
                    scaleFreqs = fliplr(scale.*(Fs/2));
                    ...
                    % [20181017] multilinear regression step using ;east absolute deviation estimator
                    RESPONSE = num2cell(negPeakTimes');
                    figure(wp*1000)
                    [IEDdata.waves(wp).beta, ~, IEDdata.waves(wp).p] = ...
                        SpatialLinearRegression(RESPONSE,P,'switch_plot',1,'Lossfun','L1'); %
            ...
            if (ishandle(wp*100) && ishandle(wp))
