...
for pt = 3 % 1:nPts
    ...
    % removing outliers.
    idx = outliers(waveBetas,[25 75],3);
    isWave(idx) = false;
    isWave(waveSpds>200) = false;
    isWave(waveBetas(1,:)>3) = false;
    ...
    if ~isempty(waveDirs)
        ...
        tH = text(mean(waveSpds(isWave)),max(hH(2).YData),sprintf('mean+/-std = %.2f+/-%.2f',mean(waveSpds(isWave)),std(waveSpds(isWave))));
        tH2 = text(mean(waveSpds(isWave)),max(hH(2).YData)-2,sprintf('N = %d total discharges. (from %d detections).',sum(isWave),length(isWave)));
        % deets
