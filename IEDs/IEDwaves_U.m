...
for pt = 2
    ...
    for fl = crashFiles(end)+1:length(dirList)
        ...
        % histogram of detections in time.
        ...
        for  wp = 1:nWavePoints
            ...
            if isempty(detectionIdcs)
                ...
                try
                    ...
                    [localMinAmps,localMinTimeIdcs] = findpeaks(smoothedWPdata>median(std(abs(smoothedWPdata),[],2)))
                end
                ...
                if plotIEDwaves
                    ...
                    [IEDdata.waves(wp).beta, IEDdata.waves(wp).V, IEDdata.waves(wp).p] = ...
                        SpatialLinearRegression(RESPONSE,P,'switch_plot',1,'Lossfun','L1','Fs',Fs); %
                    ...
                    for chz = 1:size(smoothedWPdata,1)
                        try
                            ...
                            fprintf('couldn"t find channel %d in the map.\n',chz);
                        end
                    end
